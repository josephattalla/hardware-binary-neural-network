...
def generate_verilog_rom(input_filename, output_filename):
    ...
    try:
        ...
        with open(output_filename, 'w') as v:
            v.write("module ROM_W2(\n")
            v.write("    input      [3:0]   address, \n")
            v.write("    output reg [255:0] data    \n")
            v.write("    );\n\n")
            ...
            for i in range(10):
                ...
                if i < len(weights):
                    ...
                    binary_str = weights[i][:256].zfill(256)
                else:
                    binary_str = "0" * 256
                
                v.write(f"            4'h{addr_hex}: data = 256'b{binary_str};\n")

            v.write("            default: data = 256'b0;\n")
            v.write("        endcase\n")
            v.write("    end\n\n")
            v.write("endmodule\n")
...
MY_INPUT_FILE = '.\models\weights_layer2.txt'  # Your source file
MY_OUTPUT_FILE = 'ROM_W2.v'     # The file you will copy into Vivado
