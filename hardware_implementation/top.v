...
module top (
    ...
    (* ram_style = "block" *) reg [783:0] w1_mem [0:255]; // forced to BRAM to avoid using thousands of LUTs
    ...
    neuron_pipelined #(.IN(784)) l1_inst (
        .clk    (clk),
    ...
    generate
        for (k = 0; k < 10; k = k + 1) begin : l2_gen
            neuron_pipelined #(.IN(256)) l2_inst (
                .clk    (clk),
    ...
    always @(posedge clk) begin
        case (state)
            ...
            IDLE: begin
                ...
                pipeline_cnt <= 0;
                if (start) begin
                    image_buf <= {image_buf[775:0], pixel_in};  // 784 - 8 - 1 = 775, shift left and insert new byte at LSB
